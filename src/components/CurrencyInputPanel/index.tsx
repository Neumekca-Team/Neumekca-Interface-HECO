import { Currency, Pair, TokenAmount } from '@bscswap/sdk'
...
const CurrencySelect = styled.button<{ selected: boolean }>`
  ...
  :focus,
  :hover {
    background: ${({ selected, theme }) => (selected ? theme.bgConcave : theme.bgConcaveWithPrimary)};
  }
`
...
interface CurrencyInputPanelProps {
  ...
  customBalanceAmount?: TokenAmount
}
      ...
      <Container hideInput={hideInput}>
                ...
                >
                  {!hideBalance && !!currency && selectedCurrencyBalance
                    ? (customBalanceText ?? 'Balance: ') +
                      (customBalanceAmount
                        ? customBalanceAmount?.toSignificant(6)
        ...
        <InputRow style={hideInput ? { padding: '0', borderRadius: '0' } : {}} selected={disableCurrencySelect}>
          ...
          >
            <Aligner>
              {pair ? (
                <DoubleCurrencyLogo currency0={pair.token0} currency1={pair.token1} size={21} margin={true} />
              ) : currency ? (
                <CurrencyLogo currency={currency} size={'21px'} />
              ) : null}
              {pair ? (
